...
starttime=$(date +%s)
CC_SRC_LANGUAGE=${1:-"javascript-asset"}
...
elif [ "$CC_SRC_LANGUAGE" = "javascript-asset" ]; then
	CC_SRC_PATH="../medical-chain/chaincode-javascript/"
else
...
./network.sh deployCC -ccn pacientCRUD -ccv 1 -cci InitLedger -ccl javascript -ccp ../medical-chain/chaincode-javascript/
popd
